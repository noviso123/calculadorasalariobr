...
const SEOContent: React.FC<Props> = ({ view, onNavigate }) => {
  
  // Componente auxiliar para link interno
  const InternalLink = ({ target, label }: { target: ViewType, label: string }) => (
    <button 
      onClick={() => {
        onNavigate(target);
    ...
    salary: (
      <article className="prose prose-blue max-w-none text-slate-600">
        ...
        <p className="mb-4">
          ...
          Para um planejamento financeiro completo, é essencial que você também entenda como funcionam suas <InternalLink target="vacation" label="Férias" /> e o seu <InternalLink target="thirteenth" label="13º Salário" />.
        </p>
        ...
        <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 mt-6">
          ...
          <details className="group">
            ...
            <p className="mt-2 text-sm text-slate-600 pl-4 border-l-2 border-blue-200">
              O FGTS (8%) <strong>não é descontado</strong> do seu salário. Ele é um depósito extra e obrigatório feito pelo empregador em uma conta vinculada na Caixa Econômica Federal. Por isso, ele não reduz seu salário líquido mensal, mas é vital em caso de <InternalLink target="termination" label="Rescisão de Contrato" />.
            </p>
          </details>
        </div>
        <MethodologyFooter />
    ...
    vacation: (
      <article className="prose prose-blue max-w-none text-slate-600">
        ...
        <p className="mb-4">
          ...
          O pagamento das férias deve ser feito até 2 dias antes do início do descanso, o que exige planejamento financeiro. Se você está saindo de férias, verifique também se já recebeu a primeira parcela do seu <InternalLink target="thirteenth" label="Décimo Terceiro" />.
        </p>
        ...
        <div className="bg-yellow-50 p-6 rounded-xl border border-yellow-100 mt-6">
          <p className="text-sm text-yellow-800 font-bold">
            ⚠️ Atenção ao Retorno: Ao receber o pagamento das férias adiantado, lembre-se que, ao voltar ao trabalho, você já terá recebido o salário daquele mês. É comum passar o mês seguinte ao retorno com o orçamento apertado. Use nossa <InternalLink target="salary" label="Calculadora de Salário Líquido" /> para planejar seus meses seguintes.
          </p>
        </div>
        <MethodologyFooter />
      </article>
    ...
    thirteenth: (
      <article className="prose prose-blue max-w-none text-slate-600">
        ...
        <p className="mb-4">
          ...
          Em caso de saída da empresa, você recebe o proporcional na <InternalLink target="termination" label="Rescisão" />.
        </p>
        ...
        <div className="bg-red-50 p-6 rounded-xl border border-red-100 mt-6 shadow-sm">
           ...
           <div className="text-sm text-red-800 space-y-2">
             <p>
               ...
               Recomendamos simular sua margem na nossa <InternalLink target="consigned" label="Calculadora de Consignado" /> para evitar surpresas.
             </p>
    ...
    consigned: (
      <article className="prose prose-blue max-w-none text-slate-600">
        ...
        <p className="mb-4">
          ...
          Para saber sua renda líquida exata, utilize a <InternalLink target="salary" label="Calculadora de Salário Líquido" />.
        </p>
        ...
        <p className="mb-4">
          ...
          Porém, atenção: ao optar por isso, parte do seu saldo FGTS fica bloqueado, o que pode impactar o valor disponível para saque em uma eventual <InternalLink target="termination" label="Rescisão" />.
        </p>
